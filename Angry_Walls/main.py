...
for i in range(1,5):
	if i == 2:
		...
		ext = "jpg"
	# FIX: Changed .ext to .{ext} to correctly use the variable
...
quit_game_msg = Message(WIDTH // 2, HEIGHT - 50, 24, "Press ESC or Q to Quit", tap_to_play_font, WHITE, win)
	...
	for event in pygame.event.get():
		...
		if event.type == pygame.KEYDOWN:
			...
			if event.key == pygame.K_RIGHT or event.key == pygame.K_d:
				move_right = True
				move_fx.play()
			if event.key == pygame.K_SPACE and (home_page or score_page):
				home_page = False
				score_page = False
			...
			if event.key == pygame.K_UP:
				current_width = min(current_width + 10, info.current_w)
				current_height = min(current_height + 10, info.current_h)
				win = pygame.display.set_mode((current_width, current_height), pygame.NOFRAME | pygame.SCALED | pygame.FULLSCREEN)
			if event.key == pygame.K_DOWN:
				current_width = max(current_width - 10, WIDTH // 2) # Prevent shrinking too small
				current_height = max(current_height - 10, HEIGHT // 2)
				win = pygame.display.set_mode((current_width, current_height), pygame.NOFRAME | pygame.SCALED | pygame.FULLSCREEN)
